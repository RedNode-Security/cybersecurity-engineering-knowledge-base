...
```text
DET-CLOUD-AWS_S3_PUBLIC_ACCESS_BLOCK_DISABLED
```
...
Data exposure risk may increase when public access blocks are disabled.
...
```text
public_access_block disabled on sensitive bucket
```
...
Bucket purpose and data classification are essential context.
...
```text
TEST-DET-CLOUD-AWS_S3_PUBLIC_ACCESS_BLOCK_DISABLED
```

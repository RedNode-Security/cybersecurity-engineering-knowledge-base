---
title: Webhook Secret Rotation Failure
domain: blue-team
...
# Webhook Secret Rotation Failure

## Detection ID

```text
DET-APPLICATION-WEBHOOK_SECRET_ROTATION_FAILURE
```
...
Webhook secret rotation failures can indicate integration drift or abuse.
...
```text
webhook signature failures exceed baseline
```
...
```json
{
  "event_id": "evt-app-005",
  "integration": "payments-webhook",
  "result": "failure",
...
```json
{
  "event_id": "evt-app-benign-005",
  "integration": "dev-webhook",
  "result": "failure",
  "environment": "dev"
}
```
...
A sudden signature failure spike can be a deployment issue or attempted forgery.
...
```text
TEST-DET-APPLICATION-WEBHOOK_SECRET_ROTATION_FAILURE
```
